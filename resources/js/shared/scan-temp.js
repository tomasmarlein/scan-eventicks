...
function setTorchButtonState(track) {
    try {
        ...
        torchBtn.textContent = hasTorch && torchOn ? "🔦 Zaklamp uit" : "🔦 Zaklamp";
    } catch {
    ...
    controls = await reader.decodeFromVideoDevice(deviceId, video, (result, err) => {
        if (result) {
            ...
            if (text && text !== lastValue && !submitting) {
                ...
                submitting = true;
                qrField.value = text;
                form.submit(); // Wil je eerst tonen ipv submitten? Vervang door je eigen handler.
            }
        }
...
(async () => {
    try {
        ...
        console.error(e);
        statusEl.textContent = "Kon camera’s niet ophalen. Toestemming gegeven?";
    }
})();
